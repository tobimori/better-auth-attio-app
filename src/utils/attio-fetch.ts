...
type AttioFetchOptions<T = any> = Omit<Parameters<typeof attioFetchOriginal>[0], "method"> & {
...
export async function attioFetch<T = any>(
  ...
  try {
    ...
    if (!result || !result.data) {
      const error = new Error("No data returned from Attio API") as AttioFetchError
