import {z} from "zod"
import {type Attribute, attributeSchema, listAttributesResponseSchema} from "../schemas/attribute"
...
async function ensureAttributesExist(
  ...
  if (attributesResult.error) {
    console.error(`Failed to fetch attributes for ${objectSlug}:`, attributesResult.error)
    return null
  }
  ...
  for (const [attioField, fieldSchema] of Object.entries(attioSchema)) {
    if (!existingAttributes.has(attioField)) {
      const createResult = await attioFetch({
        ...
        body: {
          data: {
            ...
            title:
              fieldSchema.title ||
              attioField.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()),
      ...
      if (createResult.error) {
        console.error(`Failed to create attribute ${attioField}:`, createResult.error)
      } else {
...
export default async function syncWebhook(request: Request): Promise<Response> {
  ...
  console.log(`[Sync Webhook] Received ${event} event for ${adapter.betterAuthModel}`)
  ...
  if (event === "delete" && data._deleted) {
    ...
    if (!recordId) {
      ...
      const attributes = await ensureAttributesExist(adapter.attioObject, adapter.attioSchema)
  ...
  if (!recordId) {
    ...
    if (orConditions.length > 0) {
      ...
      if (!searchResult.error && searchResult.data && searchResult.data.length > 0) {
        recordId = searchResult.data[0].id.record_id
        console.log(`Found existing record ${recordId} using unique attributes`)
      }
    }
  }
  ...
  console.log(`Synced ${event} to Attio ${adapter.attioObject}:`, result.data?.id.record_id)
