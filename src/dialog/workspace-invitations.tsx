...
import getWorkspaceInvitations, {
	type Invitation,
} from "../fn/workspace-invitations.server";
...
const WorkspaceInvitationsDialogContent = ({
	recordId,
}: {
	recordId: string;
}) => {
	...
	return (
		<DialogList emptyState={{ text: "No invitations found" }}>
			{results.values[`invitations_${recordId}`].invitations.map(
				(invitation) => (
					<InvitationItem
						key={invitation.id}
						invitation={invitation}
