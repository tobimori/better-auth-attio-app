...
const InvitationsWidget = ({recordId}: {recordId: string}) => {
  ...
  const capitalizedRole = mostRecentInvitation?.role
    ? mostRecentInvitation.role.charAt(0).toUpperCase() + mostRecentInvitation.role.slice(1)
    : null

  return (
    <Widget.TextWidget onTrigger={() => showWorkspaceInvitationsDialog(recordId)}>
      ...
      <Widget.Badge text={count.toString()} color={count > 0 ? "blue" : "grey"} />
    </Widget.TextWidget>
  )
}
