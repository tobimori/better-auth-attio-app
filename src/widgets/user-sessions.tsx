...
const SessionsWidget = ({recordId}: {recordId: string}) => {
  ...
  const {sessions: sessionList, activeCount} = results.values[`sessions_${recordId}`]
  ...
  const relativeTime = useRelativeTime(mostRecentSession?.updatedAt || new Date().toISOString())

  return (
    <Widget.TextWidget
      onTrigger={mostRecentSession ? () => showUserSessionsDialog(recordId) : undefined}
    >
      ...
      {mostRecentSession ? (
        <>
          ...
          <Widget.Text.Secondary>
            ...
            {mostRecentSession.ipAddress && `IP: ${mostRecentSession.ipAddress} • `}
      ...
      <Widget.Badge text={activeCount.toString()} color={mostRecentSession ? "blue" : "grey"} />
    </Widget.TextWidget>
  )
}
